...
class DDOMEngine {
    ...
    simulate(allocation) {
        ...
        const visitors = this._discover(allocation.discVar || 0);
        const trials   = this._try(visitors, allocation.tryVar || 0);
        let { orders, grossArr } = this._buy(trials, visitors, allocation.buyVar || 0);
        const mau      = this._use(orders, allocation.useVar || 0);
        const { retention, cancelArr } = this._renew(mau, allocation.renewVar || 0);
        ...
        const netNewArr = grossArr + cancelArr;
